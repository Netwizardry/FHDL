...
### 2026-06-04 (5차) UI/UX ↔ 엔진 전기능 정합 (P1~P4)
- **P1**: 피팅 UI ↔ 엔진 단절 해소 — GUI 피팅(소문자 키·list 문법)이 국부손실에
  실제 반영(h_k>0). core/fittings.py 포괄화·별칭, 유령 입력란 제거, z/하이라이터 일관화.
- **P2**: 결과 패널에 피팅 K(자동 포함)·절대해발·대기압 컬럼 추가.
- **P3**: 그래프 편집 확장 — 펌프 양정/유량, 배관(더블클릭) 길이·관경·자재·피팅 편집.
- **P4**: 단위계 IMPERIAL 표시(L/min↔GPM, MPa↔psi, m↔ft, mm↔in),
  제약 조건(유속·안전율) 편집 메뉴.
- 테스트 68개 통과.
...
### 2026-06-04 (8차) DB 수정 툴 — 부품 라이브러리 CRUD + 노드 삭제
- **library_db CRUD**: 기존 읽기 전용 → 관경·재질·피팅·펌프커브·유체에 대해
  list/upsert/delete 추가 (신설·수정·삭제로 최신 데이터 유지).
- **GUI 부품 라이브러리 관리 다이얼로그**(설정 메뉴): 5탭 테이블 CRUD,
  [저장]으로 DB 반영(upsert + 사라진 키 삭제).
- **노드 삭제**: dsl_editor.remove_node — 노드 블록 + 연결 배관·connect 동반 삭제.
  노드 편집 다이얼로그에 [삭제] 버튼(확인 후 제거).
- 테스트 +6 (라이브러리 CRUD 5 + 노드삭제 1). 총 84개 통과.

### 2026-06-04 (9차) 작업 로그 콘솔 + 단축키
- **하단 로그 콘솔**(gui/log_console.py): 타임스탬프·레벨별 색상(INFO/OK/WARNING/
  ERROR/RUN). 진행 상태·진단(오류/경고)·해석 요약·편집/저장/라이브러리 동작을 출력.
  수직 스플리터 하단 배치, 헤더에 지우기 버튼, 최대 2000줄.
- **단축키**: 실행 Ctrl+Enter/F5, 중단 Ctrl+., 저장 Ctrl+S, 열기 Ctrl+O,
  새 프로젝트 Ctrl+Shift+N, 제약 Ctrl+E, 라이브러리 Ctrl+Shift+L,
