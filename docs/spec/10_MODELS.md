...
### 2.5 Calculation Model (Calc States)
...
오류 및 경고를 일관되게 관리하기 위한 통합 진단 객체 모델입니다.
*   **DiagnosticItem:**
    *   `code`: 진단 코드 (예: `SEM001`, `WRN003`).
    *   `severity`: 위험도 (`ERROR`, `WARNING`, `INFO`).
    *   `message`: 사용자에게 표시할 진단 메시지.
    *   `node_id`: 오류가 발생한 네트워크 노드/엣지 식별자 (있을 경우).
    *   `layer`: 오류가 발생한 파이프라인 레이어 (`Syntax`, `Semantic`, `Network`, `Solver`).
