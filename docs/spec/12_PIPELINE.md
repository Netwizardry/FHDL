...
### 4.1 [S-PIP-006] JSON-RPC 메시지 스키마 명세
GUI와 Core 엔진은 다음 규격의 JSON-RPC 2.0 메시지를 통해 통신한다.

#### (1) 해석 실행 요청 (Analyze Request)
*   **Method:** `analyze`
*   **Params:**
    ```json
    {
      "project_id": "string",
      "source_code": "string", // FHDL DSL 원문
      "options": {
        "friction_model_override": "DW | HW | null",
        "calculate_sizing": "boolean"
      }
    }
    ```
*   **Result (Success):** `AnalysisResult` 객체 (5. 섹션 참조)

#### (2) 실시간 동기화 요청 (Sync Request)
*   **Method:** `sync_entity`
*   **Params:**
    ```json
    {
    ...
    ```
*   **Result (Success):** `{"status": "patched", "new_source": "string"}`

#### (3) 상태 조회 (Get Status)
*   **Method:** `get_pipeline_status`
*   **Result:** `{"state": "Calculating | Validated | Idle", "progress": 0~100}`

## 5. 데이터 모델 및 엔티티 상세 규격 (FluidSystem)
