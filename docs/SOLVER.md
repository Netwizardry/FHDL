...
- 공급수두 = **흡입수두 + 양정**. 흡입수두 = 상류 탱크 수면 수두 − **흡입관 마찰·국부손실**(없으면 펌프 고도).
- 양정 결정 우선순위:
  1. **커브 운전점** — `curve_id` 가 있고 라이브러리가 주입되면, 펌프 통과 유량에서 커브 H(Q)를 선형 보간한 값(운전점).
  2. **MANUAL `head`** — 고정 양정 주입.
  3. **auto** — 양정 0(고정수두 노드), 요약에서 권장 양정 산정(사양 선정).
